    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(authorize ->
                ...
                .formLogin(form -> form.loginPage("/loginForm") //인증을 안하고 들어오면 '/loginForm'로 이동
                        .loginProcessingUrl("/login")  // 사용자가 로그인 폼을 제출하면 "/login" URL로 요청이 전달
                        .defaultSuccessUrl("/main")  // 인증이 성공하면 "/main" URL로 리다이렉트
                ).oauth2Login(oauth2Login ->  // OAuth2 로그인 설정을 시작 (즉, 인증 성공 후 사용자의 정보를 가져오는 역할)
                        oauth2Login.loginPage("/loginForm")  // 로그인 페이지의 경로를 '/loginForm'으로 설정(사용자가 인증을 위해 접근할 URL)
                                .userInfoEndpoint(userInfoEndpoint -> // 사용자 정보 엔드포인트를 설정
                                        userInfoEndpoint.userService(pricipalOauth2UserService))  // 사용자 정보를 가져올 서비스를 pricipalOauth2UserService로 설정
